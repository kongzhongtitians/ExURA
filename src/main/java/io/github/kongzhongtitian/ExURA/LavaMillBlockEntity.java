...
public class LavaMillBlockEntity extends BlockEntity {
    private int cooldown = 0;
    private int lastWater = 0; // 改为实例变量，每个水车独立

    public LavaMillBlockEntity(BlockPos pos, BlockState state) {
        super(ExURABlockEntity.LAVA_MILL_BLOCK_ENTITY.get(), pos, state);
    }

    public static void tick(Level level, BlockPos pos, BlockState state, LavaMillBlockEntity entity) {
        ...
        if (entity.cooldown >= 40) {
            ...
            if (entity.lastWater != currentWater) {
                ...
                globals.increase("all_gp", waterDifference * 2);
